...
function ADXL345(callback)
{
	...
	this.accelAngle = [0.0,0.0,0.0];
	this.accelD_TO_G = 57.2957795131;
	this.R = 0;
	//init stuff here
	...
	function(cb) {
		self.wire.writeBytes(0x2C, [8 + 2], function(err) {
			cb(err);
		});
	}], function(err) {
		if (!err) {
...
ADXL345.prototype.measureAccel = function(callback) {
	...
	this.wire.readBytes(0x32, 6, function(err, res) {
		if (!err) {
			...
			R = Math.sqrt(Math.pow(2, self.meterPerSecSec[XAXIS]) + Math.pow(2, self.meterPerSecSec[YAXIS]) + Math.pow(2, self.meterPerSecSec[ZAXIS]));
			...
			for (var axis = XAXIS; axis <= ZAXIS; axis++) {
				...
				self.accelAngle[axis] = Math.atan(self.meterPerSecSec[axis]/R) * self.accelD_TO_G;
			}
			callback(null);
		} else {
			callback(err);
...
ADXL345.prototype.computeAccelBias = function(callback) {
	var self = this;
	function getSamples() {
		if (self.accelSampleCount < self.accelSAMPLECOUNT) {
			self.measureAccelSum(function() {
				setTimeout(getSamples, 2.5);
			});
		} else {
			for (var axis = 0; axis < 3; axis++) {
				self.meterPerSecSec[axis] = (self.accelSample[axis] / self.accelSAMPLECOUNT) * self.accelScaleFactor[axis];
